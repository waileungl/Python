...
class Users:
    ...
    def validate_registration(input):
        ...
        if len(input['location']) < 1:
            flash("A location must be selected.", "registration")
            is_valid = False
        if len(input['language']) < 1:
            flash("A language must be selected.", "registration")
            is_valid = False
        if not len(input['birthday']):
            flash("Please enter your birthday!", "registration")
            is_valid = False
        else:
            birthday = datetime.strptime(input['birthday'],"%Y-%m-%d").year
            current_time = datetime.now().year
            if current_time - birthday < 18:
                flash("You must be at least 18 years old in order to register!", "registration")
                is_valid = False
        if len(input['pokemon']) < 1:
            flash("Please select your starter pokemon!", "registration")
            is_valid = False
    ...
    def register(cls, data):
        query = "INSERT INTO users(first_name, last_name, location, language, birthday, pokemon, email, hash, created_at, updated_at) VALUES(%(first_name)s, %(last_name)s, %(location)s, %(language)s, %(birthday)s, %(pokemon)s, %(email)s, %(password)s, now(), now());"
